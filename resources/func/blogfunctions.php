...
public function ip_exist($ip){
    ...
    $ip=$db->real_escape_string($ip);
      $query=$db->query("SELECT ip_address FROM visitors_ip where  ip_address= '{$ip}' ");
      return ($query->num_rows==0) ? true : false;
  }
  public function insert_ip($ip){
        $db = new mysqli(DB_HOST,DB_USER,DB_PASS,DB_NAME);
         $ip=$db->real_escape_string($ip);
         $db->query("INSERT INTO visitors_ip SET ip_address='{$ip}'");
  }
public function total_visitors(){
    $db = new mysqli(DB_HOST,DB_USER,DB_PASS,DB_NAME);
    $query="SELECT count(*) FROM visitors_ip";
   $result=$db->query($query);
   $r=$result->fetch_row();
   $numrows=$r[0];//how many rows in visitors_ip table
    return $numrows;
     }
}
